...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    fetch(event.request)
      .then((response) => {
        // Got a fresh response from the network — use it, and update the cache
        // so the next offline fallback is also reasonably fresh.
      ...
      .catch(() => {
        ...
        // and if that specific request was never cached, fall back to the
        // cached homepage so the app at least opens instead of showing
        // a browser error.
